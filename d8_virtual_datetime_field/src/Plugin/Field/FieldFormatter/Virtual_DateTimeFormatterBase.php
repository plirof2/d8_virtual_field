...
/**
 * Base class for 'Virtual_DateTime Field formatter' plugin implementations.
 */
abstract class Virtual_DateTimeFormatterBase extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs a new Virtual_DateTimeDefaultFormatter.
   *
  ...
  protected function setTimeZone(DrupalDateTime $date) {
    if ($this->getFieldSetting('virtual_datetime_type') === Virtual_DateTimeItem::VIRTUAL_DATETIME_TYPE_DATE) {
      ...
      $timezone = VIRTUAL_DATETIME_STORAGE_TIMEZONE;
    }
    else {
